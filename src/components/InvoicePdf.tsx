...
const InvoicePdf: React.FC<InvoicePdfProps> = ({
  ...
  const calculateTotals = () => {
    ...
    // Amount payable = previous pending balance + this invoice's grand total
    const currentBalance = round2(Number(previousBalance) + grandTotal);
            ...
            {items.map((item) => {
              ...
              return (
                <View key={item.slNo} style={styles.tableRow}>
                  ...
                  <Text style={[styles.tableCell, styles.totalItemsCell]}>{hasBoxes ? (Number(item.boxes) * Number(item.itemsPerBox)) : item.quantity}</Text>
          ...
          <View style={styles.amountWordsSection}>
            ...
            <View style={styles.amountWordsRight}>
              ...
              <View style={styles.balanceRowLast}>
                <Text style={styles.balanceLabel}>Amount Payable:</Text>
