        ...
        try {
                    ...
                    if (isEditing) {
                        ...
                        const { data: invoiceData, error: invoiceError } = await supabase
                            ...
                            .insert({
                                ...
                                tax_amount: taxAmount,
                                amount_received: 0,
                                previous_balance: pendingBefore,
            ...
            try {
                ...
                const itemsForPdf = items.map((item, idx) => {
                    ...
                    // Pass boxes and itemsPerBox only when they were explicitly entered (> 0)
                    const boxes = (item.boxes != null && Number(item.boxes) > 0) ? Number(item.boxes) : undefined;
                    const itemsPerBox = (item.itemsPerBox != null && Number(item.itemsPerBox) > 0) ? Number(item.itemsPerBox) : undefined;
                    return {
                        slNo: idx + 1,
