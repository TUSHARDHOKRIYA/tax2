...
import LedgerPdf, { LedgerTransaction } from "@/components/LedgerPdf";
...
export default function SalesReport() {
    ...
    const handleDownloadLedger = async () => {
        ...
        setIsGeneratingPdf("ledger");

        try {
            // Find the earliest and latest transaction dates dynamically
            let earliestDate: Date | null = null;
            let latestDate: Date | null = null;

            const updateDates = (dateString: string) => {
                const d = new Date(dateString);
                if (!earliestDate || d < earliestDate) earliestDate = new Date(d);
                if (!latestDate || d > latestDate) latestDate = new Date(d);
            };

            invoices.forEach(inv => updateDates(inv.created_at));
            payments.forEach(p => updateDates(p.created_at));

            // Set boundaries (start at 00:00:00, end at 23:59:59)
            const sDate = earliestDate ? new Date(earliestDate) : null;
            if (sDate) sDate.setHours(0, 0, 0, 0);

            const eDate = latestDate ? new Date(latestDate) : null;
            ...
            invoices.forEach(inv => {
                ...
                if (sDate && d < sDate) {
                    openingBalance += Number(inv.total_amount);
                } else if (!eDate || d <= eDate) {
                    transactions.push({
                        date: formatISTShort(inv.created_at),
                        dateDate: d,
                        particulars: 'Sale',
                        vchType: 'Sales',
            ...
            payments.forEach(p => {
                ...
                if (sDate && d < sDate) {
                    ...
                    transactions.push({
                        date: formatISTShort(p.created_at),
                        dateDate: d,
                        particulars: (p.note || '').toLowerCase().includes('cash') ? 'By Cash' : 'By Yes Bank',
                        vchType: 'Receipt',
                        vchNo: p.id.startsWith('local-') ? Math.floor(Math.random() * 1000).toString() : p.id.split('-')[0].substring(0, 6),
                        debit: null,
            ...
            transactions.sort((a, b) => a.dateDate.getTime() - b.dateDate.getTime());
            ...
            const buyerAddressLines = selectedCompany.address
                ? selectedCompany.address.split(/\r?\n/).filter(Boolean)
                : [];

            const buyerDetails = {
                name: selectedCompany.name,
                address: buyerAddressLines,
            };

            console.log("Generating Ledger PDF...", { companyDetails, buyerDetails, transactionsCount: transactions.length });
            ...
            const filename = `Ledger_${selectedCompany.name.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', filename);
            ...
            toast({
                title: "Error",
                description: "Failed to generate or download the ledger.",
                variant: "destructive"
            });
        } finally {
            setIsGeneratingPdf(null);
                                ...
                                <div className="p-6 border-b border-border flex flex-col md:flex-row md:items-end justify-between gap-4">
                                    ...
                                    <div className="flex items-end gap-3">
                                        <Button
                                            ...
                                            disabled={isGeneratingPdf === 'ledger' || isLoading || (invoices.length === 0 && payments.length === 0)}
                                        >
                                            {isGeneratingPdf === 'ledger' ? <Sparkles className="h-4 w-4 animate-spin" /> : <FileSpreadsheet className="h-4 w-4" />}
                                            Ledger
                                        </Button>
                                        <Button
                                            variant="outline"
