...
const ItemsStep = () => {
  ...
  const confirmAddItem = useCallback(() => {
    ...
    // Only enable box tracking when user actually entered values
    const hasBoxInput = addBoxes.trim() !== '' && Number(addBoxes) > 0;
    const hasItemsPerBoxInput = addItemsPerBox.trim() !== '' && Number(addItemsPerBox) > 0;
    const boxes = hasBoxInput ? Math.max(1, Math.floor(Number(addBoxes))) : undefined;
    const itemsPerBox = hasItemsPerBoxInput ? Math.max(1, Math.floor(Number(addItemsPerBox))) : undefined;
    // Total qty = boxes × itemsPerBox when both provided, else use boxes count, else 1
    const totalQty = (boxes != null && itemsPerBox != null)
      ? boxes * itemsPerBox
      : boxes != null
        ? boxes
        : 1;
      ...
      const nextItems = items.map((i) =>
        i.item.id === addItemDialogItem.id
          ? {
            ...
            boxes: boxes ?? i.boxes,
            itemsPerBox: itemsPerBox ?? i.itemsPerBox,
          }
          : i,
      );
      setItems(nextItems);
  ...
  const confirmEditItem = useCallback(() => {
    ...
    // Only enable box tracking when user actually entered values
    const hasBoxInput = editBoxes.trim() !== '' && Number(editBoxes) > 0;
    const hasItemsPerBoxInput = editItemsPerBox.trim() !== '' && Number(editItemsPerBox) > 0;
    const boxes = hasBoxInput ? Math.max(1, Math.floor(Number(editBoxes))) : undefined;
    const itemsPerBox = hasItemsPerBoxInput ? Math.max(1, Math.floor(Number(editItemsPerBox))) : undefined;
    const totalQty = (boxes != null && itemsPerBox != null)
      ? boxes * itemsPerBox
      : boxes != null
        ? boxes
          ...
          {addItemDialogItem && (
            <div className="space-y-4 py-2">
              ...
              <div className="rounded-lg bg-muted/50 p-3 text-sm">
                {(() => {
                  const hasBoxes = addBoxes.trim() !== '' && Number(addBoxes) > 0;
                  const hasPerBox = addItemsPerBox.trim() !== '' && Number(addItemsPerBox) > 0;
                  const b = hasBoxes ? Math.max(1, Math.floor(Number(addBoxes))) : undefined;
                  const ipb = hasPerBox ? Math.max(1, Math.floor(Number(addItemsPerBox))) : undefined;
                  const qty = (b != null && ipb != null) ? b * ipb : b != null ? b : 1;
                  return (
                    <>
                      <span className="text-muted-foreground">Total: </span>
                      <span className="font-mono font-medium">
                        {b != null && ipb != null ? `${b} × ${ipb} = ` : ''}{qty} units
                      </span>
                      <span className="text-muted-foreground"> × ₹{(Number(addPricePerItem) || 0).toFixed(2)} = </span>
                      <span className="font-mono font-semibold text-foreground">
                        ₹{(qty * (Number(addPricePerItem) || 0)).toFixed(2)}
                      </span>
                    </>
                  );
                })()}
          ...
          {editDialogItemId && (() => {
            ...
            return (
              <div className="space-y-4 py-2">
                ...
                <div className="rounded-lg bg-muted/50 p-3 text-sm">
                  {(() => {
                    const hasBoxes = editBoxes.trim() !== '' && Number(editBoxes) > 0;
                    const hasPerBox = editItemsPerBox.trim() !== '' && Number(editItemsPerBox) > 0;
                    const b = hasBoxes ? Math.max(1, Math.floor(Number(editBoxes))) : undefined;
                    const ipb = hasPerBox ? Math.max(1, Math.floor(Number(editItemsPerBox))) : undefined;
                    const qty = (b != null && ipb != null) ? b * ipb : b != null ? b : 1;
                    return (
                      <>
                        <span className="text-muted-foreground">Total: </span>
                        ...
                        <span className="font-mono font-semibold text-foreground">
                          ₹{(qty * (Number(editPricePerItem) || 0)).toFixed(2)}
                        </span>
                      </>
                    );
                  })()}
